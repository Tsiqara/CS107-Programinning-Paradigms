...
static void generateRandomSentence(map<string, Definition>& grammar, vector<string> &sentence, string curNonTerminal){
  map<string, Definition>::const_iterator found = grammar.find(curNonTerminal);
  if(found == grammar.end()){
    exit(1);
  }

  Production prod = grammar[curNonTerminal].getRandomProduction();
  for (Production::iterator curr = prod.begin(); curr != prod.end(); ++curr) {
    if((*curr)[0] == '<'){
      generateRandomSentence(grammar, sentence, *curr);
    }else{
...
int main(int argc, char *argv[])
{
  ...
  for(int version = 1; version <= 3; version ++){
    ...
    generateRandomSentence(grammar, sentence,"<start>");
