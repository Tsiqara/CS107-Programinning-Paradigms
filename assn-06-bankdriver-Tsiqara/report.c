...
{
  pthread_mutex_lock(&(bank->reportTransferLock));
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&(bank->reportTransferLock));
...
{
  pthread_mutex_lock(&(bank->checkIfLastToFinish));
  bank->numberWorkersNotFinished --;
  if(bank->numberWorkersNotFinished != 0){
    pthread_mutex_unlock(&(bank->checkIfLastToFinish));
    sem_wait(&(bank->waitingToStartNextDay));
    return 0;
  }else{
    Report *rpt = bank->report;
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS) {
        // We've run out of report storage for the bank
        for(int i = 0; i < bank->numberWorkers - 1; i ++){
          sem_post(&(bank->waitingToStartNextDay));
        }
        bank->numberWorkersNotFinished = bank->numberWorkers;
        pthread_mutex_unlock(&(bank->checkIfLastToFinish));
        return -1;
    }
    /*
