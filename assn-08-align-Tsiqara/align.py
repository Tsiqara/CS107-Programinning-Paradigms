...
def findOptimalAlignment(strand1, strand2, mem):
	# if one of the two strands is empty, then there is only
	# one possible alignment, and of course it's optimal
	if len(strand1) == 0:
		lst = (' ' * len(strand2), strand2, len(strand2) * -2)
		mem[(strand1, strand2)] = lst
		return lst
	if len(strand2) == 0: 
		lst = (strand1, ' ' * len(strand1), len(strand1) * -2)
		mem[(strand1, strand2)] = lst
		return lst
	...
	# they actually match.
	try:
		lst = mem[(strand1[1:], strand2[1:])]
	except KeyError:
		lst = findOptimalAlignment(strand1[1:], strand2[1:], mem)
	
	bestWith = lst[2]
	str1 = lst[0]
	str2 = lst[1]
	if strand1[0] == strand2[0]:
		lst = (strand1[0] + str1, strand2[0] + str2, bestWith + 1)
		mem[(strand1, strand2)] = lst
		return lst # no benefit from making other recursive calls
	...
	try:
		lst = mem[(strand1, strand2[1:])]
	except KeyError:
		lst = findOptimalAlignment(strand1, strand2[1:], mem)
	...
	# opposite scenario
	try:
		lst = mem[(strand1[1:], strand2)]
	except KeyError:
		lst = findOptimalAlignment(strand1[1:], strand2,mem)
...
def printAlignment(score, out = sys.stdout):
	out.write("Optimal alignment score is " + str(score[2]) + "\n")

def getScores(strand1, strand2):
	pos = "+ "
	...
	for i in range(0, len(strand1)):
		if(strand1[i] != strand2[i] and strand1[i] != " " and strand2[i] != " " ):
			neg += "1"
		elif(strand1[i] != strand2[i] and (strand1[i] == " " or strand2[i] == " " )):
			neg += "2"
		else:
			neg += " "
...
def main():
	...
	while (True):
		...
		mem = {}
		alignment = findOptimalAlignment(strand1, strand2, mem)
		printAlignment(alignment)
		strand1 = alignment[0]
		strand2 = alignment[1]
		scores = getScores(strand1, strand2)
		positiveScores = scores[0]
		negativeScores = scores[1]
		print(positiveScores)
		print("  " + strand1)
		print( "  " + strand2)
		print(negativeScores)
		test(alignment[2], positiveScores[2:], negativeScores[2:], strand1, strand2)
