...
(define (partition pivot num-list compare)
  (if (null? num-list) '(() ())
	...
	(if (< (compare (car num-list) pivot) 0)
...
(define (intersection-points lst)
  (if 
    (< (length lst) 2) '()
    (let
      ((circle1 (car lst)) (lst2 (cdr lst)))
      (apply append (append (map (lambda (circle2) (intersect circle1 circle2)) lst2) (list(intersection-points lst2))))
    )
  )
)

(define (distance-product ptr lst)
  (apply * 
    (map 
      (lambda (x) 
        (if
...
(define (rate-points lst)
  (map (lambda (ptr) (cons (distance-product ptr lst) (list ptr))) lst)
)

(define (comp-rate-points lst1 lst2)
  (- (car lst1) (car lst2))
)

(define (sort-points lst)
  (quicksort lst comp-rate-points)
)

(define (clumped-points points)
  (prefix-of-list (map cadr (sort-points (rate-points points))) (quotient (length points) 2))
)

(define (average-point lst)
  (let ((avg-point (list (/ (apply + (map x lst)) (length lst)) (/ (apply + (map y lst)) (length lst)))))
    (cons (apply * 
      (map (lambda (p) 
        (if 
          (equal? p avg-point) 1
